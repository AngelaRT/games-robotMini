...
////////////////////////////////////////////////////////////////////////////////
void openGameInBrowser(int game){
  //game: 1=memoryGame
  string url;
  switch(game){
    case 1:
      url = "file:///home/user/Documents/pruebas-juegos-mini/memoryGame%20BROWSER/memoryGame.html";
      break;
  }
  std::stringstream openInBrowser;
  openInBrowser << "chromium-browser " << url << "&";
...
bool isAMemoryGameCommand(string msg){
  return isANumber(msg) || msg=="easy_level" 
...
void dialogCallback(const std_msgs::String::ConstPtr& msg){
  ...
  else if(last_presented=="memory_game" && isAMemoryGameCommand(msg->data)){
    status = ACTIVE;
...
void process(){
  ...
  if (status == PAUSED || status == STOPPED){
    ...
    if (requested_dialog == "presentSession"){
       ...
       play_game_std(requested_dialog,"memory_game_handler");
       string_msg.data = last_presented;
       dialog_finished_pub.publish(string_msg);;
   }
   ...
   else if (isAMemoryGameCommand(requested_dialog) && last_presented=="memory_game"){
      if(isANumber(requested_dialog) || requested_dialog=="restart"){
  ...
  }else if (isAMemoryGameCommand(requested_dialog) && last_presented=="differences_game"){
      if(isANumber(requested_dialog) || requested_dialog=="restart"){
        sendGameCommand(requested_dialog);
      }else{
        gameStarted=true;
...
int main(int argc, char **argv){
  ...
  pairFound_sub = n.subscribe("memGame_pairFound", 1, dialogCallback_memGame);
