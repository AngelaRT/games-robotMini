...
function showDifference(idSol){
	if(!$('#sol'+idSol).hasClass('solved')){
		$('#sol'+idSol).removeClass('oculta').addClass('visible').addClass('solved');
	}
	setTimeout(function(){
...
function selectNumberOfCards(level){
	switch(level){
		case 1:
			NUMBER_PAIRS=2;
			NUMBER_COLS=4;
...
function getNumDifferenceOfImage(idImg){
	switch(idImg){
		case 1:
			return 3;
		case 2:
...
function loadGame(){
	...
	if(id_game=="memGame"){
		...
		var idImg = Math.floor(Math.random()*NUMBER_DIFFERENT_IMAGES+1);
		game_pieces["idImg"]=idImg;
		var image_path = "img/"+id_game+"/differences_"+idImg;
		game_pieces["init"]=image_path+".png";
		NUMBER_DIFFS=getNumDifferenceOfImage(idImg);
		for(var i=0; i<NUMBER_DIFFS; i++)
			game_pieces["solution_"+i]=image_path+"_solution_"+i+".png";
	}
...
function buildContentGame(game_pieces){
	...
	if(id_game=="memGame"){
		...
		htmlToInsert += '<img class="visible" src="'+game_pieces["init"]+'">';
		for(var i=0; i<NUMBER_DIFFS; i++)
			htmlToInsert += '<img class="oculta solImg" id="sol'+i+'" src="'+game_pieces["solution_"+i]+'">';
		htmlToInsert += '</div>';
	}
